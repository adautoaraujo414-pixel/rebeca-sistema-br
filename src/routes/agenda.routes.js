...
router.get('/perfil', authAgenda, async (req, res) => {
  try {
    ...
    res.json({ sucesso: true, admin: { _id: a._id, id: a._id, nome: a.nome, email: a.email, nomeNegocio: a.nomeNegocio, slug: a.slug, segmento: a.segmento, telefone: a.telefone, whatsapp: a.whatsapp, logo: a.logo, descricao: a.descricao, endereco: a.endereco, cidade: a.cidade, instagram: a.instagram, plano: a.plano, config: a.config , genero: a.modoWhatsappDono?.genero || '', apelido: a.modoWhatsappDono?.apelido || '' } });
  } catch(e) { res.status(500).json({ erro: e.message }); }
});
