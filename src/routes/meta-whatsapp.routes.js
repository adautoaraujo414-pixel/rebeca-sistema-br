...
router.post('/webhook', express.json(), async (req, res) => {
  ...
  try {
    ...
    for (const msg of changes.messages) {
      ...
      // ── DEDUPLICAÇÃO PERSISTENTE: ignorar mensagem já processada ──
      try {
        const { MsgDedup } = require('../models');
        const _jaProcessou = await MsgDedup.findOne({ msgId });
        if (_jaProcessou) {
        ...
        // fallback: dedup em memória se banco falhar
        if (!global._metaMsgDedup) global._metaMsgDedup = new Map();
        if (global._metaMsgDedup.has(msgId)) {
          console.log('[MetaWA-DEDUP-MEM] Duplicada ignorada:', msgId);
          continue;
        }
