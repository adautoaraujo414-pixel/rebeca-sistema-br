...
// ══ MODULO FINANCEIRO REMOVIDO ══
// Financeiro era do Delivery — não pertence ao admin de Corridas
// Use o painel do Delivery para acessar Financeiro
function carregarFinCaixas() {
  const el = document.getElementById('page-fin-caixas');
  if (el) el.innerHTML = '<div style="text-align:center;padding:60px;color:#888"><div style="font-size:48px;margin-bottom:16px">🚫</div><h3>Módulo não disponível</h3><p style="margin-top:8px">O financeiro pertence ao sistema Rebeca Delivery.<br>Acesse o painel do Delivery para ver esses dados.</p></div>';
}
function carregarFinLucro() {
  const el = document.getElementById('page-fin-lucro');
  if (el) el.innerHTML = '<div style="text-align:center;padding:60px;color:#888"><div style="font-size:48px;margin-bottom:16px">🚫</div><h3>Módulo não disponível</h3><p style="margin-top:8px">O financeiro pertence ao sistema Rebeca Delivery.</p></div>';
}
function carregarFinContas() {
  const el = document.getElementById('page-fin-contas');
  if (el) el.innerHTML = '<div style="text-align:center;padding:60px;color:#888"><div style="font-size:48px;margin-bottom:16px">🚫</div><h3>Módulo não disponível</h3><p style="margin-top:8px">O financeiro pertence ao sistema Rebeca Delivery.</p></div>';
}
function carregarFinFornecedores() {
  const el = document.getElementById('page-fin-fornecedores');
  if (el) el.innerHTML = '<div style="text-align:center;padding:60px;color:#888"><div style="font-size:48px;margin-bottom:16px">🚫</div><h3>Módulo não disponível</h3><p style="margin-top:8px">O financeiro pertence ao sistema Rebeca Delivery.</p></div>';
}
